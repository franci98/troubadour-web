...
class PrimarySchoolRhythmExerciseGenerator
{

    public static function generateForLevel($level, Exercise $exercise) : PrimarySchoolRhythmExercise {
        ...
        return self::saveExercise([
            'primary_school_bar_info_id' => $bar_info_info->id,
    ...
    private static function saveExercise($ex, &$bars) : PrimarySchoolRhythmExercise {

        $ex = PrimarySchoolRhythmExercise::query()->create($ex);
        ...
        DB::insert("INSERT INTO primary_school_rhythm_exercise_bars (primary_school_rhythm_exercise_id, primary_school_rhythm_bar_id, seq) VALUES $tuplets");
    ...
    private static function chooseFeatureBar($feature, $spaceLeft) {
        ...
        from primary_school_rhythm_bar_occurrences o
            JOIN primary_school_rhythm_bars b on b.id = o.primary_school_rhythm_bar_id

        WHERE (b.cross_bar = 0 OR b.cross_bar IS NULL) AND o.primary_school_rhythm_feature_id = :fid AND b.length <= :len", ['fid' => $feature->id, 'len' => $spaceLeft]);
    ...
    private static function chooseFeatureBarNoRests($feature, $spaceLeft) {
        ...
        WHERE b.rests IS NULL AND (b.cross_bar = 0 OR b.cross_bar IS NULL) AND o.primary_school_rhythm_feature_id = :fid AND b.length <= :len", ['fid' => $feature->id, 'len' => $spaceLeft]);
    ...
    private static function getFeaturesForLevelAndBar($level, $bar_info_id){
      ...
      FROM primary_school_rhythm_feature_occurrences fo
        JOIN primary_school_rhythm_features f ON f.id = fo.primary_school_rhythm_feature_id
        JOIN (
          SELECT f.id as fid, MIN(b.length) as minBarLength
          FROM primary_school_rhythm_feature_occurrences fo
            JOIN primary_school_rhythm_features f ON f.id = fo.primary_school_rhythm_feature_id
            JOIN primary_school_rhythm_bar_occurrences bo ON bo.primary_school_rhythm_feature_id = f.id
            JOIN primary_school_rhythm_bars b ON b.id = bo.primary_school_rhythm_bar_id
          WHERE fo.rhythm_level = :level1 AND fo.primary_school_bar_info_id = :barinfo1 AND f.cross_bar = 0
          GROUP BY f.id
        ) v ON v.fid = f.id

      WHERE fo.rhythm_level = :level2 AND fo.primary_school_bar_info_id = :barinfo2",
    ...
    private static function getBarInfosCollection($level) {
        ...
        $infos = DB::select("SELECT bi.*, bi.probability as prob
            from primary_school_bar_infos bi where bi.min_rhythm_level <= ? and bi.id in (
            SELECT b.id
            from primary_school_bar_infos b
                join primary_school_rhythm_feature_occurrences fo on fo.primary_school_bar_info_id = b.id
    ...
    private static function getCrossBarForLevel($level, $barInfo){
        ...
        $fts = DB::select("SELECT f.id as primary_school_rhythm_feature_id
        from primary_school_rhythm_features f
            join primary_school_rhythm_feature_occurrences fo on fo.primary_school_rhythm_feature_id = f.id
        where f.cross_bar = 1 AND fo.primary_school_bar_info_id = ? AND fo.rhythm_level = ?", [$barInfo->id, $level]);
        ...
        from primary_school_rhythm_bars b
            join primary_school_rhythm_bar_occurrences bo on bo.rhythm_bar_id = b.id
        where b.cross_bar IS NOT NULL AND bo.primary_school_rhythm_feature_id IN (?)", [$ids]); // TODO
